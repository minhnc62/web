...
const INITIAN_STATE = {
  ...
  ], // gồm có id, title,descr, price, img
  cart: [], //gồm có id, title,descr, price, img, qty
  currentItem: null,
};

export const shopReducer = (state = INITIAN_STATE, action) => {
  switch (action.type) {
    case actionTypes.ADD_TO_CART:
      //Lấy dữ liệu ra từ mảng product
      const item = state.products.find((prod) => prod.id === action.payload.id);

      //Kiểm tra xem món hàng đã có trong giỏ chưa
